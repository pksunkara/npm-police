 ...
 */
check.pkg = function (pkg, local, repo, callback) {
  var pkgs = [];
  ...
  if (repo) {
    if (pkg.private && !repo.private) {
      police.winston.info('  ✗'.red.bold + ' Need to set GitHub repo ' + repo.html_url.blue + ' to ' + 'private'.yellow);
    } else if (!pkg.private && repo.private) {
      police.winston.info('  ✗'.red.bold + ' Need to set package ' + pkg.name.blue + ' to ' + 'private'.yellow);
    }
  }
    ...
    if (err) {
      ...
      fs.readFile(file, function (err, data) {
        if (err) {
          ...
          try {
            check.pkg(JSON.parse(data), path.resolve(file), null, police.exit);
          } catch (err) {
...
check.npm = function (dep, callback) {
  npmconf.load({some:'configs'}, function (er, conf) {
    ...
    police.request.get(registry + dep.name.replace('/', '%2f') + '/latest', function (err, res, body) {
